  ...
  return (
    ...
    >
        <div className='rounded-xl h-[9rem] py-1'>
            <Image 
                src={
                    category.image ? category.image : `/categories/image--${category.name.toLowerCase().split(" ").join("-")}.svg`
                }
                alt={category.name}
                width={300}
                height={200}
        ...
        <div className='py-2 px-6 flex flex-col gap-4'>
            <div>
                ...
                <p className='text-gray-600 text-sm leading-none font-semibold'>{category.description}</p>
            </div>
        </div>
    </div>
  )
